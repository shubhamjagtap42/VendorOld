...
export class ProjectComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.vendorService.getClients().subscribe(
      (data: any) => {
        ...
        this.clientInfo = this.transformProjectData(data);
    ...
    this.user.getuUser().subscribe(
      ...
      (error: HttpErrorResponse) => {
        ...
        //   severity: 'error',
        //   summary: 'Error',
        //   detail: "11Something went wrong, please try again..!!",
        // });
        window.location.reload();
      }
    );
    ...
    const categoryData = inputData.filter((data: any) => {
      ...
      for (let i = 0; i < data.businessUser.length; i++) {
        ...
        if (data.businessUser[i] === sessionStorage.getItem('email')) {
          this.allFilterProjectData.push(data);
        }
      }
    });
    return this.allFilterProjectData;
  }
  ...
  onUpload(projectData: any, file: any) {
    ...
    this.vendorService.uploadDoc(file).subscribe(
      (data: any) => {
        ...
        // console.log(this.document,'more');
      },
      (error: HttpErrorResponse) => {
        this.messageService.add({
          severity: 'error',
  ...
  updateDocument1(data: any, projectData: any) {
    ...
    this.vendorService.document(document).subscribe((data: any) => {
      // console.log(data, 'Naru More');
    });
  }
      ...
      if (file) {
        ...
        this.vendorService.addClient(this.clientForm1.value).subscribe(
          (data: any) => {
            ...
            this.onUpload(JSON.parse(data), file);
            this.clientForm1.reset();
          ...
          (error: HttpErrorResponse) => {
            if (error.status === 406) {
              this.messageService.add({
                ...
                summary: 'Warning...!!',
                detail:
                  'Duplicate project name , project code or business owner email-Id not allowed',
              });
              this.spinner.isLoading.next(false);
            } else {
              this.messageService.add({
  ...
  editProject(data: any) {
    this.vendorService.getProjectById(data.projectId).subscribe(
      (data: any) => {
        ...
        // console.log(data, ' akshay', this.newprojectData);
  ...
  onClickUpdateClient() {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        // console.log('data to be updated', this.clientForm1.value);
        // console.log(
        //   'data to be updated: ',
        //   JSON.stringify(this.clientForm1.value)
        // );
